...
//..Здесь мы создаем объект и возвращаем его
   ...
   let message = {
    ...
    // получим в переменные элементы со страницы
    let form = document.querySelector('.main-form'),
        input = document.getElementsByTagName('input'),
        statusMessage = document.createElement('div'); // создаем новый элемент
        statusMessage.classList.add('status'); // стилизуем его добавляя ему класс


    form.addEventListener('submit', function(event) {// submit - подтверждение нашей формы, это событие происходит когда наша форма
                //                                               отправляется 
                //вешать обработку события на кнопку - ошибка, обработку назначаем на форму!!!!
      
        
        event.preventDefault(); // когда у нас есть форма и у нее есть submit - это может быть input или button и если мы нажмем
        // на этот submit то у нас обновится страница , чтобы это исправить мы должны отменить стандартное поведение браузера event.preventDefault();
        // теперь страница уже не отправляет запрос  именно это нам и нужно потому что это мы будем делать с помощью AJAX 

        // Теперь мы должны пользователя уведомить как прошел наш запрос и чтобы нам его вывести нужно поместить новый элемент в нашу форму
        form.appendChild(statusMessage);
        // Создаем наш запрос, для того чтобы мы могли отправить данные на сервер
        let request = new XMLHttpRequest();
        request.open('POST', 'server.php');
        request.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded'); //..сообщает  что наш контент будет содержать данные
        // полученные из формы
        // Для того чтобы правильно отправлять данные на сервер нам необходимо чтобы в inputах стояли параметры name именно из этого 
        // атрибута name мы будем формировать ключ а значение уже введет пользователь
        // Получаем данные которые ввел пользователь при помощи объекта formdata - она создастструктуру из данных в форме ключ-значение
        let formData = new FormData(form); //то что ввел пользователь
        request.send(formData); //здесь body - данные с формы

        request.addEventListener('readystatechange', function() { // прописывается чтобы наблюдать за изменением состоянием нашего
         // запроса
        ...
        // теперь очистим все inputы
         for (let i = 0; i < input.length; i++) {
             input[i].value = '';
         }
         // после submit input полностью очистится останется только placeholder
         // сейчас мы отправили данные в обычном формате те как это воспринимает php код в своем стандартном варианте
         // однако иногда сервера (например написанные на node.js) требуют чтобы данные были в формате json
         
         
         
         // а вот как отправить в формате JSON 
         // поменяем заголовок
       //  request.setRequestHeader('Content-Type', 'application/json; charset=utf-8');
       // далее данные которые мы получили с формы нужно преобразовать в json формат
       // formData - это объект с парами ключ-значение но мы не можем сразу его перевести в json сначала создадим промежуточный объект
         // let obj = {}; //..пустой объект будем заполнять при помощи цикла forEach
         //в formData - лежат все данные которые ввел пользователь
         
        //  formData.forEach(function(value, key) {
        //      obj[key] = value; // превратили объект formData в обычный читаемый объект
        //  })

        //  let json = JSON.stringify(obj); // этот метод превращает обычные объекты в json формат
        //  request.send(json);
    ...
    // Подключаю контактную форму сам
